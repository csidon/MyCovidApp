...
void HandleCSV::initialAccountUserDBs(int uid)
{
    auto path = QStandardPaths::writableLocation(QStandardPaths::AppDataLocation);
    if (path.isEmpty()) qFatal("Cannot determine settings storage location");
    QDir d{path};
    QString filepath = returnCSVFilePath("./database/UserDoses/" + QString::number(uid) + ".csv");//creates new file
    if (d.mkpath(d.absolutePath()) && QDir::setCurrent(d.absolutePath()))
    {
        qDebug() << "Writing to " << QDir::currentPath();
        QFile f{filepath};
        ...
        {
            QTextStream stream(&f);
            stream << "doseDate" << "," << "doseManufacurer" << "," << "doseUserID" << "," << "doseIsNew" <<"\n";
        }
        f.close();
    }
    filepath = returnCSVFilePath("./database/UserTests/" + QString::number(uid) + ".csv");//creates new file
        if (d.mkpath(d.absolutePath()) && QDir::setCurrent(d.absolutePath()))
        {
            qDebug() << "Writing to " << QDir::currentPath();
            QFile f{filepath};
            ...
            {
                QTextStream stream(&f);
                stream << "testDate" << "," << "testResult" << "," << "testUserID" << "," << "testRecDate" <<"\n";
            }
            f.close();
        }
}
